...
def select_flight(ChangedFlight_Sched, AlterFlight_Sched, nextFlight, rules_df):
    ...
    DIFF_DEP = (AlterFlight_Sched['DepartureDateTime'].iloc[0] - ChangedFlight_Sched['DepartureDateTime'].iloc[0]).total_seconds()/3600
    if DIFF_DEP > constraints['MAX_DEPARTURE_DELAY']:
        # print(DIFF_DEP, AlterFlight_Sched['DepartureDateTime'].iloc[0], ChangedFlight_Sched['DepartureDateTime'].iloc[0])
        return False, 1
    ...
    if nextFlight is not None:
        # print(nextFlight, type(nextFlight))
...
def find_downline_connections(DEP_KEY, passenger_row):
